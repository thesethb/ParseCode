...
while n < len(df.Lean) - 1:
    ...
    if df.Year[n] != year or n == len(df.Lean) - 1:
        ...
        X = df[["Avg_Def_Adj","Lean","Fresh"]][m:n]
        X = sm.add_constant(X)
        y = df.Margin[m:n]
...
#df = pd.read_csv(new)
df['ratings*right'] = df.DefxYear1 * df.Party
X = df[["Lean", "Minority", "Nat_Margin", "Fresh", "Avg_Def_Adj","Midterm","Party", "Year1",'Has_Pres',"DefxParty","DefxLean","DefxMin",
        "DefxNat_Margin","DefxYear1","DefxFresh","DefxMid",'DefxHas_Pres','ratings*right']]
...
f.write(x)
X = df[["Lean", "Minority", "Nat_Margin", "Fresh", "Avg_Def_Adj",'Has_Pres']]
#X1 = df[["Lean", "Minority", "Nat_Margin", "Fresh", "Year1","Midterm","Party"]]
y = df["Margin"]
#y1 = df.Calculated_C6
#X = sm.add_constant(X)
